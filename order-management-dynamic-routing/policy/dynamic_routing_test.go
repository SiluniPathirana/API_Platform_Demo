...
// newReqCtx builds a RequestHeaderContext with the caller's org identity on
// the X-Org-Name header (as the gateway would hand it to this policy) and
// the caller's own bearer token on the incoming Authorization header.
func newReqCtx(orgID, callerToken string) *policy.RequestHeaderContext {
	...
	if orgID != "" {
		headers["x-org-name"] = []string{orgID}
	}
	return &policy.RequestHeaderContext{
		SharedContext: &policy.SharedContext{AuthContext: &policy.AuthContext{Authenticated: true}},
...
func TestOnRequestHeaders_NoOrgHeader_Returns503(t *testing.T) {
...
func TestGetPolicy_CustomOrgIDHeader(t *testing.T) {
	exchangeServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		json.NewEncoder(w).Encode(map[string]string{"access_token": "backend-token-abc"})
	}))
	defer exchangeServer.Close()
	...
	reqCtx := &policy.RequestHeaderContext{
		...
		Headers: policy.NewHeaders(map[string][]string{
			"authorization":     {"Bearer caller-token-123"},
